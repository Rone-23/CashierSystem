...
public class MakeTransaction{
    ...
    public void makeTransaction( OpenTransaction openTransaction ){
        if (!openTransaction.getReturnStatus()) {
            ...
            Item[] keptItems=openTransaction.getItemsInTransaction().values().toArray(new Item[0]);
            Item[] returnedItems=openTransaction.getReturnedItems().values().toArray(new Item[0]);
            Item.setTotalAmountZero();
            StringBuilder receipt = ReturnReceipt.makeReceipt(keptItems, returnedItems, openTransaction.getTransactionDateTime(), openTransaction.getTransactionID());
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(String.format("%s/receipt_returned_%d.txt",FILE_PATH,openTransaction.getTransactionID())))) {
                writer.write(receipt.toString());
                System.out.println("Receipt successfully saved to " + FILE_PATH);
            } catch (IOException e) {
                System.err.println("Error writing receipt file: " + e.getMessage());
